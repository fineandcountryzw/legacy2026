...
async function createTables() {
  try {
    ...
    // Create user_development_assignments table
    await sql`CREATE TABLE IF NOT EXISTS user_development_assignments (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    ...
    console.log('✅ user_development_assignments table created');
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_user_dev_assignments_user_id ON user_development_assignments(user_id)`;
    await sql`CREATE INDEX IF NOT EXISTS idx_user_dev_assignments_dev_id ON user_development_assignments(development_id)`;
    console.log('✅ Indexes created');
    
    // Insert default admin (self-referencing will fail, so use a workaround)
    await sql`INSERT INTO users (id, email, first_name, last_name, role, is_active)
