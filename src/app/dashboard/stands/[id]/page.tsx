...
import { Building2, MapPin, User, FileText, History, PieChart, ShieldCheck } from "lucide-react"
...
import { MOCK_STANDS, MOCK_TRANSACTIONS } from "@/lib/constants"
import { DataTable } from "@/components/data-table"
import { Badge } from "@/components/ui/badge"

export default function StandDetailPage() {
    const { id } = useParams()
    const stand = MOCK_STANDS.find(s => s.id === id) || MOCK_STANDS[0]

    const paymentColumns: any[] = [
        { header: "Date", accessorKey: "date" },
        { header: "Reference", accessorKey: "reference" },
        { header: "Description", accessorKey: "description", cell: (row: any) => row.description || "Payment for stand" },
                        ...
                        <div>
                            ...
                            <div className="mt-2 space-y-3">
                                ...
                                <div className="flex justify-between border-b pb-2">
                                    ...
                                    <span className="font-medium">{stand.standTypeLabel}</span>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="mt-2 space-y-3">
                                <div className="flex justify-between border-b pb-2">
                                    ...
                                    <Money amount={stand.agreedPrice} />
                                </div>
                                <div className="flex justify-between border-b pb-2">
                                    <span className="text-slate-500">Total Paid</span>
                                    <Money amount={stand.totalPaid} className="text-emerald-600" />
                                </div>
                                <div className="flex justify-between border-b pb-2">
                                    <span className="text-slate-500">Remaining Balance</span>
                                    <Money amount={stand.balance} className="text-red-500" />
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        },
        {
            ...
            content: (
                <div className="space-y-4">
                    ...
                    <DataTable columns={paymentColumns} data={MOCK_TRANSACTIONS} />
                </div>
            )
        },
        {
            ...
            content: (
                <div className="space-y-6">
                    ...
                    <DataTable columns={allocationColumns} data={[
                        { id: "1", transactionId: "t-1", type: "stand_price", payTo: "developer", amount: 9000 },
                        { id: "2", transactionId: "t-1", type: "commission", payTo: "fine_country", amount: 750 },
                        { id: "3", transactionId: "t-1", type: "admin_fee", payTo: "fine_country", amount: 250 },
                    ]} />
                    <div className="flex justify-end p-4 bg-slate-50 rounded-lg">
                        <div className="text-right space-y-1">
                            ...
                            <Money amount={10000} className="text-xl" />
                        </div>
                    </div>
                </div>
            )
