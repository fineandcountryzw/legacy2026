...
export async function POST(
  ...
  try {
    ...
    // Complete the stand
    const result = await completeStand(standId, user.id, ipAddress);
    
    return NextResponse.json({
      success: true,
      stand: result,
      message: `Stand ${result.standNumber} marked as completed`,
    });
  } catch (error) {
    console.error('Error completing stand:', error);
    return NextResponse.json(
